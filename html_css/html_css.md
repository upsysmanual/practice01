...
CSS 에서 사용할 수 있는 color 값 기준
- color 의 이름을 직접 지정하는 경우: red, blue, grey, white...
- 16진수 (HEX): 6자리의 알파벳과 숫자를 조합해서 사용. #a1eb35, #ffffff...
- 10진수 (RGB): 0~255의 10진수 값 세 개를 함수 rgb()의 매개변수로 사용하는 방식. grb(121, 211, 58)...

### 데이터의 단위
- 1 bit: 2진수 1자리를 저장할 수 있는 용량, 데이터 크기의 최소단위
- 1 byte: 8 bit, 정보 표현의 최소단위, 표현가능 개수는 2^8=256 개
- 알파벳 1자는 1 byte
- 한글 1자는 2 byte

### 색상의 표현방법
- RGB 방식: red, green, blue 3가지의 색을 조합해서 가산혼합으로 색을 표현, 화면 상의 색상을 주로 표현
- CMYK 방식: cyan, magenta, yellow, k(black) 4가지의 색을 조합해서 감산혼합으로 색을 표현, 인쇄물의 색상을 주로 표현

### RGB 방식의 색상 표현 방법
- 24 bit 트루컬러 - red, green, blue 를 각각 1byte 씩 사용해서 색을 표현 2^24 개의 색상을 표현 가능

### HEX 방식: 16진수 색상 표현 방법
- 0~9 는 숫자로 표현, 10~15 는 알파벳 a~f 로 변환해서 사용
- 2진수 4 bit 를 16진수 1bit 로 변환
- 2진수 24자리를 16진수 6자리로 변환 (red/red, green/green, blue/blue)

### 알고 있으면 편한 색상
- #ffffff : white, #000000 : black
- #555555, #f9f9f9 : 2자리씩 끊어서 숫자 또는 문자가 동일한 경우는 주로 회색 계열
- #555555 는 #555, #55ffdd 는 #5fd 로 줄여서 쓸 수 있다. (같은 숫자를 표현하는 숫자가 같으면 삭제할 수 있음)

## 이미지의 종류

- 비트맵
  - 점으로 표현된 데이터. 확대하면 깨짐
  - .jpg: 색표현이 우수 카메라로 촬영한 사진에 사용하기 적합. 투명 배경을 사용할 수 없다.
  - .png: 디자인 이미지에 사용하기 적합. 투명한 배경을 사용할 수 있다. 
  - .gif: 색표현 품질이 낮음. 투명한 배경 사용 가능. 애니메이션 가능

- 벡터
  - 좌표로 표현된 데이터. 무한히 확대해도 안깨진다.
  - .svg: 
...
- alpha
'''
- opacity 의 사용: 0.0 ~ 1.0 사이값을 사용. 속성이기 때문에 HTML element 자체를 투명하게 적용.
div {
  opacity: 0.5
}

- transparent 의 사용: 지정된 색상에 투명한 속성을 추가함.
div {
  background: transparent;
}

- alpha 의 사용: rgb 값 뒤에 0.0 ~ 1.0 사이값을 추가해서 사용. 지정된 색상에 투명한 속성을 추가함.
div {
  color: rgba (a100, 509, 240, 0.5)
}
'''