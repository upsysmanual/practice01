...
- 변수 (Variable):
  ...
  - 변수 선언 예약어 (Keyword)
    - var (ES5 에서 사용): 데이터 타입을 전혀 구분하지 않음. 데이터 변경이 자유로움.
    - let (ES6 에서 사용): 데이터 타입을 전혀 구분하지 않음. 데이터 변경이 자유로움.
    - const (ES6 에서 사용): 데이터 타입을 전혀 구분하지 않음. 변수이지만 데이터 변경이 불가능함. 긴 소수나 큰 수를 반복해서 사용할 경우의 불편함을 개선하기 위해 그 수를 대신해서 사용할 수 있도록 고정한 수. (원주율 등의 무리수)
